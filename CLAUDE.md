...
- **Both refs outlive their owner here, and Riverpod 3 throws for it.** The widget `ref` is backed by `BuildContext`, so `dispose()` must use a notifier captured in `didChangeDependencies`, never `ref.read`. The notifier's own `ref` is separate but just as fragile: every `state =` or `ref.invalidate` that follows an `await` needs an `if (!ref.mounted) return` guard, because logout invalidates the provider mid-flight and a disposed `Ref` throws rather than no-opping. Regression test: `test/widgets/notifications_screen_teardown_test.dart`.
...
- **The badge has only `poll()`** — no forced variant. The three user actions that change the count (`markAllRead`, `clearAll`, a committed delete) invalidate the provider outright, and a feed load hands its count over via `setCount`. `poll()`'s first call lands while `build()` is still in flight, so it awaits `future` rather than racing a second request for the same integer.
- **`poll()` returns true only on a strict rise from an existing `AsyncData`** — that is the "something arrived" signal, and the missing baseline is what stops the cue firing on every cold launch. The rise drives `Sfx.newNotification` from the poller, and the open notifications screen listens for the same rise to `silentRefresh()` + `markAllRead()`.
- **`silentRefresh()` refuses to run while a delete is queued.** Reloading inside the undo window puts the dismissed row back under the user's finger, and flushing the queue first would destroy the undo they were just offered.
- The unread count already rides along with every feed page, so `refresh()`/`silentRefresh()` push it to the badge via `setCount` instead of spending a second request. Never from `build()` — writing another provider mid-build is what Riverpod forbids outright.
...
- Do NOT touch the widget `ref` in `State.dispose()` — it is backed by `BuildContext`; capture the notifier in `didChangeDependencies` and call that
